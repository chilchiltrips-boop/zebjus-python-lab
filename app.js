(function(){
  ...
  let editor=null,worker=null,ws=null,running=false,cameraRunning=false,currentCameraIndex=null,cameras=[],liveMode=false,liveCode="",liveNeedsHand=false,liveNeedsFace=false,liveNeedsCamera=false,liveTimer=null,liveSessionId=0,lintTimer=null,lintSeq=0,lintWaiters=new Map(),editorIssue=null,pendingCircuitCode=null;
...
while True:
  ...
  const COMPONENT_CATALOG=[
    ...
    {className:"I2CDevice",label:"Generic I²C Device",group:"Universal Buses",interface:"I²C shared bus + address",max:16,prefix:"i2cdev",status:"ready",pinCost:{out:2}},
  ...
  function applyCircuitCode(code){if(typeof code!=="string"||!editor||code===getCode())return false;setCode(code);if($("saveState"))$("saveState").textContent="Circuit synced";scheduleHardwareCards(code);return true;}
  function receiveCircuitCode(code){if(typeof code!=="string"||!editor||code===getCode())return;if(running){pendingCircuitCode=code;if($("saveState"))$("saveState").textContent="Circuit update queued";log("Circuit update queued. It will be applied after the program stops.");return;}applyCircuitCode(code);}
  function applyPendingCircuitCode(){if(running||typeof pendingCircuitCode!=="string")return false;const code=pendingCircuitCode;pendingCircuitCode=null;return applyCircuitCode(code);}
  window.addEventListener("storage",e=>{if(e.key==="zebjus.lab.code")receiveCircuitCode(e.newValue);});
  window.addEventListener("zebjus-circuit-code",e=>receiveCircuitCode(e?.detail?.code));
  ...
  function createWorker(){
    ...
    worker.onmessage=e=>{
      ...
      if(m.type==="ready"){
        ...
        running=false;updateRunControls();endHardwareRun().finally(applyPendingCircuitCode);
        const issue={
        ...
        if(liveMode&&running){
          liveTimer=setTimeout(()=>runLiveCycle().catch(err=>{
            liveMode=false;running=false;updateRunControls();endHardwareRun().finally(applyPendingCircuitCode);log("Live loop error: "+(err?.message||err));badge($("pythonStatus"),"Python ready","ok");
          }),70);
        }else{
          running=false;updateRunControls();clearEditorIssue();
          endHardwareRun().finally(()=>{log("Program finished. RGB/motor outputs are OFF; OLED keeps the last displayed frame.");applyPendingCircuitCode();});
          badge($("pythonStatus"),"Python ready","ok");
        }
      }
    ...
    worker.onerror=e=>{running=false;updateRunControls();endHardwareRun().finally(applyPendingCircuitCode);log("Worker error: "+e.message);badge($("pythonStatus"),"Python error");};
  }
  ...
  function suggestedHardwareArgs(item,src){
    ...
    if(item.className==="I2CDevice"){const existing=allPinEntries(src).find(e=>e.shareKey&&String(e.shareKey).startsWith("i2c:0:"));if(existing){const parts=existing.shareKey.split(":");return `0x40, ${parts[2]}, ${parts[3]}, 400000, 0`;}const free=PIN_HINT_ORDER.OLED.filter(p=>!used.has(p));return free.length>=2?`0x40, ${free[0]}, ${free[1]}, 400000, 0`:null;}
  ...
  async function runCode(){
    ...
    if(needsCamera && !directCameraOk){
      if(!isEmbedded){
        running=false;updateRunControls();applyPendingCircuitCode();
  ...
  function markKitFailure(reason=""){
    ...
    if(kitFailureCount>=KIT_FAILURE_LIMIT){
      ...
      for(const q of displayHardwareQueues.values()){
        // Preserve genuine module/wiring faults; every other pause is caused by the lost transport.
        if(q.pauseReason!=="device"){q.paused=true;q.pauseReason="transport";}
      }
