...
async function createLandmarker(delegate){
  const c=cfg();
  const {FilesetResolver,HandLandmarker}=await import(c.moduleUrl);
...
async function ensureModel(){
  ...
  catch(e){console.warn("MediaPipe GPU unavailable, using CPU",e);handLandmarker=await createLandmarker("CPU");}
  state.ready=true;
...
function draw(canvas,lm){
  ...
  for(const p of lm){ctx.beginPath();ctx.arc(p.x*w,p.y*h,2.7*d,0,Math.PI*2);ctx.fill();}
}

function emit(){
  window.dispatchEvent(new CustomEvent("zebjus-ai-state",{detail:{detected:state.detected,fingers:state.fingers,side:state.side}}));
}
...
async function start(video,canvas,deviceId=""){
  ...
  const videoConstraint=deviceId?{deviceId:{exact:deviceId},width:{ideal:960},height:{ideal:540}}:{facingMode:"user",width:{ideal:960},height:{ideal:540}};
  ...
  await ensureModel();
  state.running=true;lastTime=-1;loop(video,canvas);
...
window.ZebjusAI={state,start,stop,getSnapshot:()=>({detected:state.detected,fingers:state.fingers,side:state.side}),getDeviceId:()=>currentDeviceId};
