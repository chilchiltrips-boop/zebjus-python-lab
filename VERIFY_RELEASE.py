...
for h in ('index.html','settings.html','camera-bridge.html'):
    ...
    if '?v=6.2.2' in text or '?v=6.1' in text: raise SystemExit(f'Stale cache-buster remains in {h}')
...
# Firmware support
fwpath=ROOT/'esp32_firmware/ZEBJUS_Kit_Universal_Hardware_Bridge_WiFi_v2_3.ino'
if not fwpath.exists(): raise SystemExit('Firmware v2.3 sketch missing')
fw=fwpath.read_text()
for marker in ('struct CounterSlot;','struct RotarySlot;','struct TM1637Slot;','struct LCD1602Slot;','X-Zebjus-Token','ver", "2.3.0"','No ultrasonic echo received','DHT11_MIN_READ_MS=1200UL','tmWriteByte','tm1637Api','lcd1602Api','/api/tm1637','/api/lcd1602','transactionBidirectionalMode'):
...
if 'ZEBJUS_Kit_Universal_Hardware_Bridge_WiFi_v2_3.ino' not in workflow: raise SystemExit('GitHub Action targets stale firmware')
...
assert bc['ui_version']=='6.3.0' and bc['firmware']=='2.3.0' and bc['examples_count']==40
print(f'ZEBJUS v6.3 release verification PASS ({len(js_files)} JS, {len(examples)} examples)')
