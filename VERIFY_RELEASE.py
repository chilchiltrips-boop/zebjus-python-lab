...
VERSION='6.7.0'; FW_VERSION='2.5.0'
...
required_js={'app.js','kit-client.js','py-worker.js','settings.js','ai.js','camera-bridge.js','component-library.js','circuit-sync.js','circuit.js','schematic.js','firmware-updater.js'}
...
if len(examples)!=41: raise SystemExit(f'Expected 41 examples, found {len(examples)}')
...
for m in ('q.pauseReason!=="device"','pendingCircuitCode','Circuit update queued','applyPendingCircuitCode'):
    if m not in app: raise SystemExit('Missing v6.7.0 runtime fix: '+m)
...
for m in ('VERSION:"6.7.0"','FIRMWARE:"2.5.0"','COMPONENTS=','BOARD_PINS=','c.asset2d=`./${c.slug}-2d.svg`'):
...
if './esp32-devkit-2d.svg' not in (ROOT/'circuit.html').read_text(): raise SystemExit('Circuit board asset is not flat/root-level')
...
# 46 root-level 2D SVG assets and readable pin labels.
svg=sorted(ROOT.glob('*.svg'))
if len(svg)!=46: raise SystemExit(f'Expected 46 root 2D SVG assets, found {len(svg)}')
...
for name in ('tm1637-2d.svg','lcd1602-2d.svg','ultrasonic-2d.svg','dht11-2d.svg','esp32-devkit-2d.svg'):
...
for m in ('function kitKey(','function selectedKit(','function tokenForKit(','new Uint8Array(await file.arrayBuffer())'):
    if m not in fwjs: raise SystemExit('Firmware Center v6.7.0 fix missing: '+m)
...
if bc.get('ui_version')!=VERSION or bc.get('firmware')!=FW_VERSION or bc.get('examples_count')!=41: raise SystemExit('BUILD_CHECK version/count mismatch')
if not bc.get('features',{}).get('reference_ui_adoption') or not bc['features'].get('web_firmware_update') or not bc['features'].get('pin_hotspot_wiring'):
    raise SystemExit('BUILD_CHECK missing v6.7.0 features')
...
print(f'ZEBJUS v{VERSION} release verification PASS ({len(js_files)} JS, {len(examples)} examples, 45 circuit components, {len(svg)} 2D SVG assets, firmware v{FW_VERSION}; browser UI {browser_state})')
