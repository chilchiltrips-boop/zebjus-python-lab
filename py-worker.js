...
class LED:
    ...
    def blink(self,count=3,interval=.5):
        for _ in range(int(count)):
            self.on(); time.sleep(float(interval))
            self.off(); time.sleep(float(interval))
...
class HandResult:
    ...
    def __repr__(self):
        return f"HandResult(detected={self.detected}, fingers={self.fingers}, side='{self.side}')"

class HandDetector:
    def read(self):
        return HandResult(_ai_state["detected"],_ai_state["fingers"],_ai_state["side"])

class Camera:
    def __init__(self,index=0): self.index=int(index)
    def read(self):
        if _current_frame is None:
            raise RuntimeError("No camera frame available. Enable camera and Run again.")
        return _current_frame.copy()

def show(image,title="OpenCV Output"):
    import cv2
    ok,buf=cv2.imencode(".png",image)
    if not ok: raise RuntimeError("Could not encode OpenCV image")
    data="data:image/png;base64,"+base64.b64encode(bytes(buf)).decode()
    postMessage(to_js(
        {"type":"image","dataUrl":data,"title":str(title)},
        dict_converter=js.Object.fromEntries
    ))

z=types.ModuleType("zebjus")
z.LED=LED; z.Motor=Motor; z.Servo=Servo; z.sleep=sleep
z.__all__=["LED","Motor","Servo","sleep"]
...
zc.Camera=Camera; zc.show=show
zc.__all__=["Camera","show"]
...
async function prepareRun(m){
  const code=m.code||"";
  const needsCv=/\bimport\s+cv2\b|\bfrom\s+zebjus_cv\s+import\b|\bimport\s+zebjus_cv\b|\bCamera\s*\(/.test(code);
...
_current_frame=None
  ...
  if(needsCv && m.frame?.data && m.frame?.width && m.frame?.height){
    pyodide.globals.set("__fw",Number(m.frame.width));
    pyodide.globals.set("__fh",Number(m.frame.height));
    pyodide.globals.set("__fd",m.frame.data);
...
initialize().catch(err=>{
  postMessage({type:"error",text:"Python init failed: "+(err?.message||err)});
});