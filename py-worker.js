...
class Camera:
    ...
    def read(self):
        if _current_frame is None: raise RuntimeError("No camera frame. Enable camera and press Run again.")
...
async function prepareRun(msg){
  const code=msg.code||"";
  if(/\b(import\s+cv2|from\s+zebjus_cv\s+import|import\s+zebjus_cv)\b/.test(code)){
    postMessage({type:"status",text:"Loading OpenCV…",mode:"warn"});
    await pyodide.loadPackage(["numpy","opencv-python"]);
  }
  await pyodide.loadPackagesFromImports(code);

  pyodide.globals.set("__stdin_text",String(msg.stdin||""));
  pyodide.globals.set("__ai_detected",!!msg.aiState?.detected);
  pyodide.globals.set("__ai_fingers",Number(msg.aiState?.fingers)||0);
  pyodide.globals.set("__ai_side",String(msg.aiState?.side||""));
...
_current_frame=None
  ...
  if(f&&f.data&&f.width&&f.height){
    pyodide.globals.set("__frame_w",Number(f.width));pyodide.globals.set("__frame_h",Number(f.height));pyodide.globals.set("__frame_data",f.data);
    await pyodide.runPythonAsync(`
import numpy as np, cv2
_raw = __frame_data.to_py() if hasattr(__frame_data,"to_py") else list(__frame_data)
_arr = np.asarray(_raw,dtype=np.uint8).reshape(int(__frame_h),int(__frame_w),4)
...
self.onmessage=async e=>{
  ...
  if(m.type==="run"){
    ...
    }catch(err){postMessage({type:"error",text:String(err)});}
  }
};
