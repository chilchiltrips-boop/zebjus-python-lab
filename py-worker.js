...
class Motor:
    ...
    def forward(self,speed=50):
        _send("MOTOR_SET",id=self.id,speed=max(0,min(100,int(speed))))
...
class Servo:
    ...
    def write(self,angle=90):
        _send("SERVO_SET",id=self.id,angle=max(0,min(180,int(angle))))
...
za.HandDetector=HandDetector
za.HandResult=HandResult
...
_ai_state={
    "detected":bool(__ai_detected),
    "fingers":int(__ai_fingers),
    "side":str(__ai_side)
}
_current_frame=None
  `);
  ...
  if(f?.data&&f?.width&&f?.height){
    pyodide.globals.set("__fw",Number(f.width));
    pyodide.globals.set("__fh",Number(f.height));
    pyodide.globals.set("__fd",f.data);
...
self.onmessage=async e=>{
  ...
  if(m.type==="init"){
    version=m.pyodideVersion||version;
    try{
      await initialize();
    }catch(err){
      postMessage({
        type:"error",
        text:"Python init failed: "+(err?.message||err)
      });
    }
  }
